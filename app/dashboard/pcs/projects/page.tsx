...
export default function PcsProjectsPage() {
  ...
  const stats = {
    ...
    planning: projects.filter((p: PcsProject) => p.status === 'PLANNING').length,
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
        ...
        <StatCard label="规划中" value={stats.planning} />
            ...
            <table className="w-full text-sm">
              <thead className="border-b bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-center font-medium text-gray-500 whitespace-nowrap">进度</th>
              ...
              <tbody>
                {filteredProjects.map((project: PcsProject) => (
                  <tr key={project.id} className="border-b last:border-0 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-xs font-medium ${PRIORITY_CLASS[project.priority]}`}>
                        {PRIORITY_ZH[project.priority]}
                      </span>
                    </td>
                    <td className="px-4 py-3">
